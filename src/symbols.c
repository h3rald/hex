...
int hex_symbol_index(hex_context_t *ctx)
{
    ...
    result = hex_push_integer(ctx, result);
    HEX_CLEANUP(ctx);
    return result;
}
...
int hex_symbol_args(hex_context_t *ctx)
{
    ...
    else
    {
        ...
        int result = hex_push_quotation(ctx, quotation, ctx->argc);
...
int hex_symbol_run(hex_context_t *ctx)
{
    ...
    int result = hex_push_quotation(ctx, quotation, 3);
    HEX_CLEANUP(ctx);
    return result;
}
...
int hex_symbol_q(hex_context_t *ctx)
{
    ...
    *quotation = *item;
    ...
    int outcome = HEX_PUSH(ctx, result);
    HEX_CLEANUP(ctx);
    return outcome;
}
